...
labels_path = os.path.join(parent_dir, DATA_FOLDER, "labels.csv")

labels = pd.read_csv(labels_path)
grade_ii_index = np.where(labels["Grade_Label"] == GRADE_II)[0]
...
def extract_tumors(mask, full):
    ...
    def remove_small_object(mask):
        blobs = measure.label((mask > 0) * 1.0, background=0)
