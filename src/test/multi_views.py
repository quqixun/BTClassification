...
# i_range = [cp[0] - distance, cp[0] + distance]
# j_range = [cp[1] - distance, cp[1] + distance]
# k_range = [cp[2] - distance, cp[2] + distance]
# ni, nj, nk = np.meshgrid(i_range, j_range, k_range, indexing="ij")
# ijks = np.zeros(list(ni.shape) + [3])
# ijks[..., 0] = ni
# ijks[..., 1] = nj
# ijks[..., 2] = nk
# ijks = np.reshape(ijks, [-1, 3]).astype(int)
